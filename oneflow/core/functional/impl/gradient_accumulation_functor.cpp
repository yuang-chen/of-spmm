...
class GradAccRepeatFunctor {
 public:
  ...
  struct GradAccRepeat {
    Maybe<AttrMap> operator()(int32_t repeat_num) {
  ...
  Maybe<Tensor> operator()(const std::shared_ptr<Tensor>& in, int32_t repeat_num) const {
    constexpr auto* GetAttrs = CACHED_FUNCTOR_PTR(GradAccRepeat);
...
class GradAccCollectFunctor {
 public:
  ...
  struct GradAccCollect {
    Maybe<AttrMap> operator()(int32_t collect_num) {
      MutableAttrMap attrs;
  ...
  Maybe<Tensor> operator()(const std::shared_ptr<Tensor>& in, int32_t collect_num) const {
    constexpr auto* GetAttrs = CACHED_FUNCTOR_PTR(GradAccCollect);
    const auto attrs = *JUST(GetAttrs(collect_num));
    return OpInterpUtil::Dispatch<Tensor>(*op_, {in}, attrs);
  }
...
class GradAccPackFunctor {
 public:
  ...
  struct GradAccPack {
    Maybe<AttrMap> operator()(int32_t pack_num) {
      MutableAttrMap attrs;
  ...
  Maybe<Tensor> operator()(const std::shared_ptr<Tensor>& in, int32_t pack_num) const {
    constexpr auto* GetAttrs = CACHED_FUNCTOR_PTR(GradAccPack);
    const auto attrs = *JUST(GetAttrs(pack_num));
    return OpInterpUtil::Dispatch<Tensor>(*op_, {in}, attrs);
  }
...
class GradAccUnpackFunctor {
 public:
  ...
  struct GradAccUnpack {
    Maybe<AttrMap> operator()(int32_t unpack_num) {
      MutableAttrMap attrs;
  ...
  Maybe<Tensor> operator()(const std::shared_ptr<Tensor>& in, int32_t unpack_num) const {
    constexpr auto* GetAttrs = CACHED_FUNCTOR_PTR(GradAccUnpack);
    const auto attrs = *JUST(GetAttrs(unpack_num));
    return OpInterpUtil::Dispatch<Tensor>(*op_, {in}, attrs);
  }
